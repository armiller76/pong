...
auto VulkanSwapchain::create_() -> void
{
    ...
    const auto formats = device_.physical_device().getSurfaceFormatsKHR(surface_.get());
    ...
    const auto chosen_format = choose_surface_format_(formats);
    surface_format_ = chosen_format.format;
    color_space_ = chosen_format.colorSpace;
...
auto VulkanSwapchain::destroy_views_() -> void
{
    image_views_.clear();
    images_.clear();
}
