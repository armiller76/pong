...
- Prompt: Templates are instantiated in the translation unit where they are called. If your load logic is in resource_manager.cpp, but you call manager.load<Mesh>(...) from pong.cpp, will the compiler be able to find the definition? Do you need to mark these as explicit specializations in the header, or move the logic into the header?

how does static_assert differ from arm::ensure?

Prompt for you: In load(Mesh mesh), you are using std::move(mesh) into try_emplace. If the meshes_.find(key) check at the top of the function discovers that the mesh is already loaded, what happens to the mesh object that was passed into the function? Is it still moved, or is it destroyed normally when the function returns?